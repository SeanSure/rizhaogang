    ...
    <div class="viewer">
      <div class="btn">
        ...
        <button v-if="hasLabel"
                @click.stop="hideLabels">取消标注</button>
        <button v-else
                @click.stop="showLabels">开启标注</button>
        ...
        <el-color-picker v-model="plEmissive"
                         @active-change="changeplEmissive"></el-color-picker>
        <el-color-picker v-model="waterColor"
                         @active-change="changeWaterColor"></el-color-picker>
        <el-slider v-model="plOpacity"
                   :step="0.01"
                   :min="0"
                   :max="1"
                   @input="changePLOpacity"></el-slider>
      </div>
  ...
  methods: {
    init() {
      ...
      devices.forEach(v => {
        ...
        div.style = 'font-size:16px;position:absolute;left:32px;top:32px;width: max-content;';
        divs.push(div);
      })
      ...
      divChild.setAttribute('style', `color:white;width:500px;height:360px;background: rgba(0, 0, 0, 0.6);transform: translate(60%, -60%);`)
